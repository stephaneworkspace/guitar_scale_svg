...
fn note(note: &str) -> std::io::Result<()> {
    ...
    for v_s in vec_scale {
        let draw: DrawScale = DrawScale::new(v_s, Theme::Light(Instrument::Piano));

        let svg = format!("{}",draw.draw_base_vertical());
        let mut buffer = File::create(format!("temp_scales_piano_svg/scale_v_light_{}_{}.svg", v_s.scale_type.get_name_short(), &tonic.data_name()))?;
